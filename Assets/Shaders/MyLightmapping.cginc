...
Interpolators MyLightmappingVertexProgram(VertexData v)
{
    ...
    v.vertex.z = v.vertex.z > 0 ? 0.0001 : 0;//It's need to make this work on all machines

    //UnityObjectToClipPos using double matrix mul: Unity_ObjectToWorld and after that UNITY_MATRIX_VP 
    //Because of that colors from surface was not extracted during baking
    //i.pos = UnityObjectToClipPos(v.vertex);

    i.pos = mul(UNITY_MATRIX_VP, float4(v.vertex.xyz, 1));
